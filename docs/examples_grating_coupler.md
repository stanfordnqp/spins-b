...
We generate a 2d design by extruding the 1D optimized grating coupler
design. In the example file the extrude length is 12 um. To generate
this GDS we run:
...
If for some reason an optimization is terminated, it can be resumed by
running:
...
For the sake of an example, let\'s adjust the optimization for a 60%
partial etch grating coupler in 300 nm thick silicon nitride on 3um of
buried oxide layer with air-cladding. We\'ll also adjust the source to
be a normally incident at 1300 nm.
...
![Explanation of source angle rotation
parameters.](_static/figures/beam_rotation.png)
:::
...
Accordingly, to change the number of continuous or discrete optimzation
iterations we adjust this argument where this function is called in the
run_opt function:
...
Likewise, the minimum feature size in the optimization is set here as
well.
...
Once an optimization has completed in the discretization stage, a GDS
...
Minimizing back reflections is set by simply turning on the flag at the
